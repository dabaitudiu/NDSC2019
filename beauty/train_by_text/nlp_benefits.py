...
tmp_y = np.asarray(data['Benefits']).astype(int)
y = []
for i in range(len(tmp_y)):
    y.append(to_categorical(tmp_y[i],num_classes=7)[0])
y = np.array(y)
...
checkpoint = ModelCheckpoint("saved_models/nlp_benefits/{}_best_model.hdf5".format("hi"), monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
