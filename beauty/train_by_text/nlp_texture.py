...
data = pd.read_csv("numerical_textures.csv")
...
tmp_y = np.asarray(data['texture_numerical']).astype(int)
y = []
for i in range(len(tmp_y)):
    y.append(to_categorical(tmp_y[i],num_classes=9))
y = np.array(y)
...
checkpoint = ModelCheckpoint("saved_models/nlp_benefits/{}_best_model.hdf5".format("color"), monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
...
# word_init_embed = init_embedding(word2vec_model)