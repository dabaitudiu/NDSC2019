...
data = pd.read_csv("numerical_colors.csv")
...
if finished:
    train_x_word = pickle.load(open("word2id.pkl","rb"))
else:
...
for i in range(len(tmp_y)):
    y.append(to_categorical(tmp_y[i],num_classes=39))
y = np.array(y)
...
checkpoint = ModelCheckpoint("saved_models/nlp_benefits/{}_best_model.hdf5".format("brand"), monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
